...
Sphere::Sphere(const Vector3D &center, double radius, const Material & material) : Body(material, std::make_unique<NormalizeInterpolator>(), center - Vector3D(1,1,1) * radius, center + Vector3D(1,1,1) * radius), center(center), radius(radius), radiusSquared(radius * radius) {}
