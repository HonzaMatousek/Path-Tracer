...
Triangle::Triangle(const Vector3D &a, const Vector3D &b, const Vector3D &c, const Material &material) : Body(material, std::make_unique<FlatInterpolator<Vector3D>>((b - a).Cross(c - a).Normalize()), a.Min(b).Min(c), a.Max(b).Max(c)), a(a), b(b), c(c), u(b - a), v(c - a), normal(u.Cross(v)) {
